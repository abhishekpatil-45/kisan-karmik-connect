...
const SearchFilters = ({
  ...
  return <form onSubmit={handleSubmit(handleSearch)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Keyword Search */}
        <div className="space-y-2">
          <Label htmlFor="keyword">Keyword</Label>
          <Input id="keyword" placeholder="Search by keyword..." {...register('keyword')} />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select onValueChange={value => setValue('crop', value)} defaultValue={initialFilters.crop || "all-crops"}>
            <SelectTrigger>
        ...
        {searchTarget === 'laborer' && <>
            <div className="space-y-2">
              ...
              <Select onValueChange={value => setValue('preferred_work_type', value)} defaultValue={initialFilters.preferred_work_type || "any-work-type"}>
                <SelectTrigger>
                  <SelectValue placeholder="Any work type" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="any-work-type">Any work type</SelectItem>
                  {workTypes.map(type => <SelectItem key={type} value={type}>
                      {type}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2 flex items-center">
              <div className="flex items-center space-x-2">
                <Checkbox id="can_relocate" onCheckedChange={checked => setValue('can_relocate', !!checked)} />
